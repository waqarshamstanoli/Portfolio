  ...
  <v-footer color="primary">
    <v-container fluid class="pa-md-8 pa-4">
      <v-row>
        <v-col cols="12" lg="4" md="12" sm="12">
          <v-form method="POST" action="https://formspree.io/f/xeqydqrl" v-model="valid" @submit.prevent="submitForm" class="pa-5" style="border: 1px solid white; border-radius: 0px">
            <v-container>
              <v-row>
                <v-col cols="12" md="10">
                  <v-text-field outlined tile dense v-model="formData.name" color="white" class="white-input" :counter="10" name="Name" :rules="nameRules" :label="$t('name')"  hide-details required></v-text-field>
                </v-col>
                <v-col cols="12" md="10">
                  <v-text-field outlined tile dense v-model="formData.email" color="white" class="white-input" :rules="emailRules" name="Email" :label="$t('email')" hide-details required></v-text-field>
                </v-col>
                <v-col cols="12" md="10">
                  <v-btn tile type="submit" outlined :disabled="!valid" color="white" class="white--text">{{$t("submit")}}</v-btn>
                </v-col>
              </v-row>
            </v-container>
          </v-form>
        ...
        <v-col cols="12" lg="8" md="12" sm="12">
          <v-row>
            <v-col cols="12" lg="4" md="3" sm="6" class="pb-0" :class="{ '': !$vuetify.breakpoint.smAndDown, custom: $vuetify.breakpoint.smAndDown }">
              <h4 class="white--text oswald-text ">{{ $t("services") }}</h4>
              <v-list color="transparent" dense>
                <v-list-item link v-for="(item, index) in company" :key="index" :to="item.link" class="white--text oswald-text text-body-2 pl-0 pr-0">
                  {{ $t(item.title) }}
                </v-list-item>
              </v-list>
            </v-col>
            <v-col cols="12" lg="4" md="3" sm="6" class="pb-0" order-lg="2" :class="{ '': !$vuetify.breakpoint.smAndDown, custom: $vuetify.breakpoint.smAndDown }">
              <h4 class="white--text oswald-text">{{ $t("have_questions") }}</h4>
              <v-list color="transparent" dense>
                <v-list-item two-line class="grey--text text--lighten-5 text-body-2 pl-0 pr-0">
                  <v-list-item-icon class="mr-2 mt-4">
                    <a href="tel:+923458112216">
                      <v-icon color="white">mdi-phone</v-icon>
                    </a>
                  </v-list-item-icon>
                  <v-list-item-content>
                    <v-list-item-title>
                      <a href="tel:+923458112216" class="white--text oswald-text">+92 923458112216 </a>
                    </v-list-item-title>
                    <v-list-item-subtitle class="white--text oswald-text"> (9:00am - 5:00pm PKT) </v-list-item-subtitle>
                  </v-list-item-content>
                </v-list-item>
                <v-list-item two-line class="white--text oswald-text text-body-2 pl-0">
                  ...
                  <v-list-item-content class="pt-0">
                    <a href="https://wa.me/3458112216">
                      <v-list-item-title>
                        <a href="https://wa.me/3458112216" class="white--text oswald-text"> Whatsapp </a>
                      </v-list-item-title>
                      <v-list-item-subtitle class="white--text oswald-text"> Worldwide chat with us </v-list-item-subtitle>
                    </a>
                  </v-list-item-content>
                </v-list-item>
                <v-list-item two-line class="white--text oswald-text text-body-2 pl-0">
                  ...
                  <v-list-item-content class="pt-0">
                    ...
                    <v-list-item-title>
                      <a href="mailto:madinatechinnovations@gmail.com" class="white--text oswald-text"> madinatechinnovations@gmail.com </a>
                    </v-list-item-title>
                  </v-list-item-content>
                </v-list-item>
              </v-list>
            </v-col>
            <v-col cols="12" lg="4" md="3" sm="6" class="pb-0" order-lg="1" :class="{ '': !$vuetify.breakpoint.smAndDown, custom: $vuetify.breakpoint.smAndDown }">
              <h4 class="white--text oswald-text">{{ $t("industries") }}</h4>
              <v-list color="transparent" dense>
                <v-list-item link v-for="(item, index) in policies" :key="index" :to="item.link" class="white--text oswald-text text-body-2 pl-0 pr-0">
                    {{ $t(item.title)}}
                </v-list-item>
              </v-list>
            </v-col>
      ...
      <v-row>
        <v-col cols="12" lg="4" md="12" sm="12">
          <div :class="{ 'mt-0': !$vuetify.breakpoint.mdAndUp, 'social-links': $vuetify.breakpoint.mdAndUp }" style="display: flex; gap: 10px">
            <a href="https://www.facebook.com/BimmerTechLLP/" v-if="this.$vuetify.theme.dark">
              <svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect x="0.5" y="0.5" width="43" height="43" rx="21.5" stroke="white" />
                <path d="M25.8086 23.375H23.1719V31.25H19.6562V23.375H16.7734V20.1406H19.6562V17.6445C19.6562 14.832 21.3438 13.25 23.9102 13.25C25.1406 13.25 26.4414 13.4961 26.4414 13.4961V16.2734H25C23.5938 16.2734 23.1719 17.1172 23.1719 18.0312V20.1406H26.3008L25.8086 23.375Z" fill="white" />
              </svg>
            </a>
            <a v-else href="https://www.facebook.com/BimmerTechLLP/">
              <svg width="44" height="44" viewBox="0 0 44 44" fill="none" class="ml-4" xmlns="http://www.w3.org/2000/svg">
                <rect x="0.5" y="0.5" width="43" height="43" rx="21.5" stroke="white" />
                <path d="M25.8086 23.375H23.1719V31.25H19.6562V23.375H16.7734V20.1406H19.6562V17.6445C19.6562 14.832 21.3438 13.25 23.9102 13.25C25.1406 13.25 26.4414 13.4961 26.4414 13.4961V16.2734H25C23.5938 16.2734 23.1719 17.1172 23.1719 18.0312V20.1406H26.3008L25.8086 23.375Z" fill="white" />
              </svg>
            </a>
            <a v-if="this.$vuetify.theme.dark" href="">
              <svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect x="0.5" y="0.5" width="43" height="43" rx="21.5" stroke="white" />
                <path
    ...
    <v-dialog color="blue" v-model="errorDialog" max-width="400">
      <v-card>
        ...
        <v-card-text> Form submission failed. Please try again later.. </v-card-text>
        <v-card-actions>
  ...
  data() {
    return {
      ...
      policies: [
        ...
        {
          title: "healthcare_pharmaceutical",
          // link: "cookiespolicy"
        },
        {
          title: "retail_cpg",
    ...
    async submitForm() {
      try {
        ...
        if (response.ok) {
          ...
          (this.emailRules = false), (this.nameRules = false);
        } else {
